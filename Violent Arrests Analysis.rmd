...
merge(
  data1 %>% 
  filter(ChargeOrder==1) %>% #you're counting every charge as its own unique case 
...
merge(
  data1 %>% 
  filter(ArrestBorough == 'M', ChargeOrder==1) %>% #you're counting every charge as its own unique case 
  mutate(offense_year = ifelse(offense_year == "2021", "2021 to date", offense_year)) %>% 
  mutate(`Year` = factor(offense_year, levels = c('2018', '2019', '2020', '2021 to date'))) %>% 
  group_by(`Year`) %>% 
  summarise(`VFOs` = n_distinct(ArrestID),
...
#MSS: I wouldn't use data2 as the base for the cat or top tables -- since the # of violent arrests is by defendant, if a single person has multiple cases pending, then the number of arrests will be double counted here - if you look at the total number of unique violent arrests associated with a Dany case, you'll notice there are only 5,823 in the dataset, but the cat table in the report counts 8,597 -- similarly, the category is by case not by person...I added a field to in the data1 table (in the query) that indicates the top charge per PERSON (rather than case) across all cases they had pending at the time of the offense i.e. if an person is at liberty on two DANY cases, one which is for a misdemeanor, and one for a felony, the felony charge matters more for our purposes -- SEE cat2 and top2 tables for corrections
